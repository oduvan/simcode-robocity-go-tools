...
// client library and has no replace and no indirect deps. robocity-sim overrides the client library with
// its local engine-backed copy via a temporary go.work, so this runs fully offline
// even though github.com/oduvan/simcode-go v0.0.1 is never downloaded.
